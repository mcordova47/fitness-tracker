...
todaysSession userId = do
  ...
  join <$> for (hush raw) \{ body } ->
    for (Nullable.toMaybe (unsafeCoerce body :: _ SessionRaw)) \session -> do -- TODO: Use argonaut instead of `unsafeCoerce`
      date <- liftEffect $ JSDate.parse session.date
      pure session { date = date }
...
lastExercise userId kind = do
  ...
  pure $ hush raw >>= \{ body } -> Nullable.toMaybe (unsafeCoerce body :: _ Exercise) -- TODO: Use argonaut instead of `unsafeCoerce`

maxSet :: String -> String -> Aff (Maybe Set)
maxSet userId kind = do
  raw <- Affjax.get json $ "/" <> userId <> "/workouts/max_set" <> "?kind=" <> kind
  pure $ hush raw >>= \{ body } -> Nullable.toMaybe (unsafeCoerce body :: _ Set) -- TODO: Use argonaut instead of `unsafeCoerce`
